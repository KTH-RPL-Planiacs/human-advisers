...
import queue
from game_synth.helpers import powerset, create_guard, sog_fits_to_guard, group_and_flip
...
def create_game(mdp, dfa):
    ...
    synth.add_node(synth_init, player=1)
    synth.graph['init'] = synth_init

    # dfa and mdp atomic propositions
    dfa_ap = dfa.graph['ap']
    mdp_ap = mdp.graph['ap']
    human_ap = list(set(dfa_ap).difference(set(mdp_ap)))
    joined_ap = mdp_ap + human_ap
    synth.graph['mdp_ap'] = mdp_ap
    synth.graph['human_ap'] = human_ap
    ...
    while not que.empty():
        ...
        if synth.nodes[synth_from]['player'] == 2:
            results = {}
            for opt in powerset(human_ap):
                # generate guard for chosen option
                opt_guard = create_guard(opt, human_ap)
                results[opt_guard] = []
        ...
        if synth.nodes[synth_from]['player'] == 0:
            for synth_succ in synth.nodes[synth_from]['res']:
                ...
                if not synth.has_node(synth_succ):
                    synth.add_node(synth_succ, player=1)
